...
| **Class dump** | At every server start, when the bridge is on: every class of the five roots the mods check (`CfgVehicles`, `CfgMagazines`, `CfgNonAIVehicles`, `CfgAmmo`, `cfgWeapons`) with its parent and its game name in the `original` and the `english` stringtable column, into `$profile:OpenZone\classes.tsv`. The server reads every `stringtable.csv` it can open in its loaded archives (`OZ_StringTables`); a class whose key no table holds gets the name the server itself resolves. The bridge turns the file into the class index every editor on the admin site checks and searches against (measured: 12 937 classes, 6 tables, about a second) |
