...
class OZ_Module : CF_ModuleWorld
{
    ...
    override void OnMissionStart(Class sender, CF_EventArgs args)
    {
        ...
        // Перелік класів сервера для моста й редакторів на ньому -- лише
        // коли міст увімкнено (секунда-дві старту), і до старту мостового
        // клієнта: міст читає файл при першому опитуванні цього сервера.
        if (s.Bridge.Enabled)
        {
            OZ_ClassDump.Write(OZ_Const.CLASSES);
            OZ_Log.Info("classes: " + OZ_ClassDump.Summary());
        }
